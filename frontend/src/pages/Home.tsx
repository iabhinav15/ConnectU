...
const Home = () => {
    ...
    const confirmDelete = async () => {
        setShowModal(false);
        if(postIdToDelete && user && user.token) {
    ...
    const confUnfriend = async () => {
        ...
        try {
            const res = await apiRequest({
                ...
                token: user?.token,
                method: "POST",
                data: { id: friendIdToRemove, userId: user?._id },
            });
            if (res.success === true) {
    ...
    const removeSentrequest = async (id: string) => {
        try {
            const res = await apiRequest({
                        ...
                        <form onSubmit={handleSubmit(handlePostSubmit)}
                            ...
                            <div className='flex items-center justify-between py-4'>
                                <label htmlFor="imgUpload" className='flex items-center gap-1 text-base text-ascent-2  hover:text-ascent-1 cursor-pointer'>
                                    <input type="file" id="imgUpload" className='hidden' onChange={(e) => { if(e.target.files) {setFile(e.target.files[0])} }} data-max-size='5120' accept='.jpg, .png, .jpeg' />
                                    <BiImages />
                                    <span>Image</span>
                                </label>

                                <label className='flex items-center gap-1 text-base text-ascent-2  hover:text-ascent-1 cursor-pointer' htmlFor='videoUpload'>
                                    <input type="file" id="videoUpload" className='hidden' onChange={(e) => { if(e.target.files) setFile(e.target.files[0]) }} data-max-size='5120' accept='.mp4, .mkv, .avi, .wav' />
                                    <BiSolidVideo />
                                    <span>Video</span>
                                </label>

                                <label className='flex items-center gap-1 text-base text-ascent-2  hover:text-ascent-1 cursor-pointer' htmlFor='vgifoUpload'>
                                    <input type="file" id="vgifoUpload" className='hidden' onChange={(e) => { if(e.target.files) setFile(e.target.files[0]) }} data-max-size='5120' accept='.gif' />
                                    <BsFiletypeGif />
                                    <span>Gif</span>
                                </label>
                                <div>
